...
public class GameManager {
    ...
    public void moveAi(long numberOfSleep) {
        for (int i = 0; i < ais.size(); i++) {
            ...
            /*int j = checkAiConllisionAis(i);
            if (j != -1) {
                random = new Random();
                ais.get(i).setOrientation(random.nextInt(4));
                ais.get(j).setOrientation(random.nextInt(4));
            }*/
...
/*    private int checkAiConllisionAis(int i) {
        for (int j = 0; j < ais.size(); j++) {
            if (i != j) {
                if (ais.get(i).getRectangle().intersects(ais.get(j).getRectangle())) {
                    return j;
                }
            }
        }
    ...
    public void saveGame() {
        random = new Random();
        codeSave = ((char) (random.nextInt(26) + 65)) + "" +
        ...
        try {
            ...
            fos.close();
            System.out.println("file save thanh cong! " + "D:/" + codeSave);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
